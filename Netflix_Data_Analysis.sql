...
group by director,country --unique values
...
from netflix_genre ng   --inner join since we dont know whch is movie or tv show
inner join netflix_country nc on ng.show_id=nc.show_id
inner join netflix n on ng.show_id=nc.show_id
where ng.genre='Comedies' and n.type='Movie'   --filter on comedy and movie
group by  nc.country
...
, cte2 as (
select *
, ROW_NUMBER() over(partition by date_year order by no_of_movies desc, director) as rn --if i put comma director after desc it means director with lower ascii value J will come first
from cte
--order by date_year, no_of_movies desc
)
select * from cte2 where rn=1
--order by is not allowed in cte or subquery imp
